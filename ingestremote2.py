...
class RestructurePackage:
    ...
    def unique_directory_path(self, directory_path):
        # If there is already a folder in the output directory with the same name
        orig = directory_path
        counter = 2
        while os.path.exists(directory_path):
            directory_path = orig + f"_{counter}"
            counter += 1
    ...
    def file_type(self, file_path):
        ...
        if result.returncode == 0:
            result = result.stdout.strip()  # Use strip() to remove any trailing newline
            nb_streams = int(result)
        else:
            nb_streams = 0

        # Stream duration
        # Define the ffprobe command and arguments
        command = [
            "ffprobe",
            "-loglevel", "quiet",
            file_path,
        ...
        if result.returncode == 0:
            output_lines = result.stdout.strip().split('\n')  # Use strip() to remove any trailing newline
        ...
        for foldername, subfolders, filenames in os.walk(input_folder_path, topdown=False):
            ...
            for file in filenames:
                if self.mac_system_metadata(file):
                    ...
                    filetype = self.file_type(os.path.join(foldername, file))
                    if filetype == "objects":
                        if object_counter == 0:
                            output_folder_path = os.path.join(output_directory, output_package_name, "objects",
                                                              output_subfolder_name)
                            output_folder_path = self.unique_directory_path(output_folder_path)
                            object_counter += 1
                        else:
                            output_folder_path = os.path.join(output_directory, output_package_name, "objects",
                                                              output_subfolder_name)
                        ...
                        if metadata_counter == 0:
                            output_folder_path = os.path.join(output_directory, output_package_name, "metadata", "logs",
                                                              output_subfolder_name)
                            output_folder_path = self.unique_directory_path(output_folder_path)
                            metadata_counter += 1
                        else:
